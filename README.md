...
两者都使用本程序支持的 HID 标识与输出报告协议：

| 连接方式 | VID:PID | 说明 |
| --- | --- | --- |
| USB 有线 | `258A:010C` | |
| Bluetooth Low Energy | `3554:FA07` | 可能显示为 `T100 5.0` |

`T100` 是设备向 macOS 暴露的控制器识别信息，并非实体键盘型号。其他使用相同
VID/PID 和报告协议的键盘也可能适用，但仍需实机确认；使用不同标识或协议的设备
目前不会被程序识别。
...
新版首次启动会停止并移除旧的 `local.codex.t100-logo-white` 后台服务，并继续读取
旧版保存的连接偏好。旧的 `T100 Logo 白色呼吸.app` 不会被自动删除，可以手动移入
废纸篓。
...
后台程序随用户登录启动并保持运行，平时处于空闲状态，只监听兼容键盘的连接和 Mac
唤醒事件，不会持续向键盘发送指令。发生以下事件时，它会短暂发送恢复报告：

- 后台服务启动；
- 兼容键盘连接或重新连接；
- Mac 从睡眠中唤醒。

每次触发会以 50 ms 间隔发送约 3 秒，即约 60 次。重复发送用于覆盖 macOS 在设备
初始化期间再次写入绿色指示状态的情况。报告只解除指示灯覆盖，使键盘恢复此前由
用户手动设置并保存在键盘中的 LOGO 灯效。
...
./keyboard-logo-fix --uninstall # 卸载新版和旧版后台服务
...
需要安装 Xcode Command Line Tools。默认生成同时支持 Apple Silicon 和 Intel Mac
的通用程序：
...
`com.ikuyu.keyboard-logo-fix.plist` 是 App 位于系统“应用程序”文件夹时可使用的
手动 LaunchAgent 模板。
...
| 文件 | 职责 |
| --- | --- |
| `src/main.c` | 命令行参数解析与各入口分发 |
| `src/keyboard.c` | HID 设备匹配与发送解锁报告 |
| `src/daemon.c` | 后台服务：运行循环、设备与唤醒回调 |
| `src/service.c` | LaunchAgent 的安装与移除 |
| `src/settings.c` | 连接偏好的读写 |
| `src/ui.c` | 连接选择窗口与结果提示 |
| `src/platform.c` | 进程启动、路径拼接、XML 转义 |
| `src/app_config.h` | Bundle 标识与共享常量 |
